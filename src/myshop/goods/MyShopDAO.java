...
public class MyShopDAO {
	...
	public List getAlbums(int start, int end) {
		...
		try {
			...
			String sql;
			sql = "select goods_code, goods_img, goods_name, goods_price, rownum from (select * from Goods order by goods_code desc) where rownum>=? and rownum<=?";
			pstmt = conn.prepareStatement(sql);
	...
	public List getAlbums(int start, int end,String filter) {
		...
		try {
			...
			String sql;
			sql = "select goods_code, goods_img, goods_name, goods_price, rownum from (select * from (select * from Goods order by goods_code desc) order by goods_price asc) where rownum>=? and rownum<=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
			rs = pstmt.executeQuery();
			...
			String sql;
			sql = "select goods_code, goods_img, goods_name, goods_price, rownum from (select * from (select * from Goods order by goods_code desc) order by goods_price desc) where rownum>=? and rownum<=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
			rs = pstmt.executeQuery();	
	...
	public List getAlbums(int start, int end,String choice,String search) {
		...
		try {
			...
			String sql;
			sql = "select goods_code, goods_img, goods_name, goods_price, rownum from (select * from Goods where " +choice+ " like'%"+search+"%'order by goods_code desc) where rownum>=? and rownum<=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
			rs = pstmt.executeQuery();
